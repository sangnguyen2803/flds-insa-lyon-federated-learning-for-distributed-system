...
class Datasets:
    ...
    @classmethod
    def HEART_DISEASE_UCI(
        cls,
        path: str = "../data",
        filename: str = "heart_disease_uci.csv",
        test_size: float = 0.2,
        seed: int = 42,
    ) -> DataContainer:
        """Load the Heart Disease UCI dataset from a local CSV file.

        This loader converts the original target ``num`` into a binary
        classification task: ``0`` means no disease, ``>0`` means disease.
        """
        csv_path = os.path.join(path, "heart_disease_uci", filename)
        df = pd.read_csv(csv_path)
        if "num" not in df.columns:
            raise ValueError("Heart disease CSV must include a 'num' column.")

        df = df.replace("?", np.nan)
        if "id" in df.columns:
            df = df.drop(columns=["id"])

        y_raw = pd.to_numeric(df["num"], errors="coerce").fillna(0)
        ...
        for col in X_df.columns:
            ...
            if numeric_col.notna().mean() >= 0.9:
                X_df[col] = numeric_col.fillna(numeric_col.median())
            else:
...
Datasets._DATASET_MAP = {
    ...
    "heart_disease_uci": Datasets.HEART_DISEASE_UCI,
